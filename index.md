...
[Add a background image to a TableView](#Add-a-background-image-to-a-TableView)
[Add a background image to a table cell](#Add-a-background-image-to-a-table-cell)
[Add a header to TableView](#Add-a-header-to-a-TableView)
[Change the status bar style](#Change-the-status-bar-style)
[Modify the border of a TextView or TextField](#Modify-the-border-of-a-TextView-or-TextField)
[Convert Unix timestamp to Date](#Convert-Unix-timestamp-to-date)
[Get a user's timezone](#Get-a-users-timezone)
[Move a view up only when the keyboard covers an input field](#Move-a-view-up-only-when-the-keyboard-covers-an-input-field)
[Calculate elapsed time](#Calculate-elasped-time)
[Conver a date string to a Date](#Convert-a-date-string-to-a-Date)
[Make dynamically sized TableView cells](#Make-dynamically-sized-TableView-cells)
[Navigate to a specific TabBar tab](#Navigate-to-a-specific-TabBar-tab)
[Detect UITabBar Selected Index/ Item Changes that is set Programmatically](#Detect-UITabBar-Selected-Index/-Item-Changes-that-is-set-Programmatically)
[Get UIColor from HEX](#Get-UIColor-from-HEX)
[Lock orientation to portrait mode](Lock-orientation-to-portrait-mode)
